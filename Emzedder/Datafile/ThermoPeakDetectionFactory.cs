...
internal class ThermoPeakDetectionFactory
{
    ...
    {
        double numerator = profilePeak.Sum(p => p.Intensity * p.Mz);
        double denominator = profilePeak.Sum(p => p.Intensity);

        double centroidMz = Math.Round(numerator / denominator, 4);

        double maxIntensity = Math.Round(profilePeak.Max(p => p.Intensity), 4);
